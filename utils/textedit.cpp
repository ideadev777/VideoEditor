...
TextEdit::TextEdit( QWidget *parent,MyGraphicsTextItem* item)
    : QMainWindow(parent)
{
    m_item = item ;
	...
	textEdit->setStyleSheet("QTextEdit{background-color:grey;}");
...
{
	if( actionRedo->isEnabled() || actionUndo->isEnabled() )
	{
		if( QMessageBox::question(this, "Confirm" , "Save changes?" ) == QMessageBox::Yes )
		{
			save() ;
		}
	}
...
void TextEdit::setupEditActions()
{
	...
	a = actionSave = new QAction(QIcon::fromTheme("Save", QIcon(rsrcPath + "/filesave.png")),
		tr("&Save"), this);
...
void TextEdit::setupTextActions()
{
	...
	actionBackColor = new QAction(pix, tr("&Back Color..."), this);
	connect(actionBackColor, SIGNAL(triggered()), this, SLOT(backColor()));
	tb->addAction(actionBackColor);
	menu->addAction(actionBackColor);
...
void TextEdit::backColor()
{
	CxTextItem* item = (CxTextItem*)(m_item->parentItem());
	QColor col = QColorDialog::getColor(item->brush().color(), this);
	if (!col.isValid())
		return;
...
void TextEdit::bgcolorChanged(const QColor &c)
{
	QPixmap pix(16, 16);
	pix.fill(c);
	actionBackColor->setIcon(pix);
}
...
void TextEdit::showMe(MyGraphicsTextItem *item)
{
...
//    QMessageBox::information(NULL,"","A") ;
	...
	CxTextItem* p = (CxTextItem*)(item->parentItem()) ;
	bgcolorChanged(p->brush().color()) ;
//    load(item->toHtml()) ;
	if( isVisible() ) QApplication::setActiveWindow(this) ;
	else show() ;
}

void TextEdit::save()
{
//	m_item->setPlainText(textEdit->toHtml()) ;
	m_item->setHtml(textEdit->toHtml()) ;
}